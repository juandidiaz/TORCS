...
public class JugadorVelocidad extends Controller {
	...
	private static QTableVelocidad qtablaV = new QTableVelocidad();
...
//			        
		// build a CarControl variable and return it
		Action action = new Action();
		int estado = this.getEstadoActualDireccion(sensors);
		int estadoV = this.getEstadoActualVelocidad(sensors);
		int movimiento = qtabla.obtenerMejorMovimiento(estado);
		int movimientoV = qtablaV.obtenerMejorMovimiento(estadoV);
		steer = Datos.direccion[movimiento];

		float accel, brake;
		
		float accel_brake[]=Datos.accel_freno[movimientoV];
		accel=accel_brake[0];
		brake=accel_brake[1];
	...
	public Integer getEstadoActualVelocidad(SensorModel sensors) {
